		...
		switch (type) {
			case "major_i":
				...
				result.addAll(
					graduationUtil.getRecommendMajorSubjects
						("major_i", user.getDepartment().getName(), courses)
				);
				break;
			case "major_s":
				//전공선택인 경우
				...
				result.addAll(
					graduationUtil.getRecommendMajorSubjects
						("major_s", user.getDepartment().getName(), courses)
				);
				break;
			case "culture":
				//교양인 경우
			...
			case "culture_b":
				...
				if (courses.stream()
					...
					.noneMatch(course -> course.getDomain().contains("EnglishFoundation") ||
						// 기초교양 1. EnglishforAcademicPurposes
			...
			case "culture_e":
				...
				if (essentialCultures.size() >= 4) {
					...
					log.info("[ 교양 추천 ] 모든 상명핵심역량 교양을 만족했습니다."); //모든 영역을 들었을 경우
				break;
			...
			case "culture_s":
				...
				if (balanceCultures.size() >= 3) {
					...
					log.info("[ 교양 추천 ] 모든 균형 교양을 만족했습니다."); //모든 영역을 이수했을 경우
				break;
		...
		cultures
			.forEach(culture -> {
				...
				if (culture.getType().equals(subject.getType())) {
					...
					log.error("[ Subject 통합 - Culture ] ERROR : Type이 맞지 않음 [{}] Culture -> {}, Subject -> {}",
						subject.getName(), culture.getType(), subject.getType());
				}
			});
		...
		majors
			.forEach(major -> {
				...
				if (major.getType().equals(subject.getType())) {
					...
					log.error("[ Subject 통합 - Major ] ERROR : Type이 맞지 않음 [{}] Major -> {}, Subject -> {}",
						subject.getName(), major.getType(), subject.getType());
				}
			});
	}
}
